...
if ($Help) {
    ...
    Write-Host ""
    Write-Host "Usage: .\setup-secrets-fixed.ps1 [options]"
    Write-Host ""
    Write-Host "Options:"
...
This file can be safely deleted after testing."
    ...
    try {
        git add $testFile
        $commitMessage = "Test CI/CD pipeline - $(Get-Date -Format 'yyyy-MM-dd HH:mm')"
        ...
        try {
            ...
            Write-Host "   Or visit: $repoUrl/actions" -ForegroundColor Gray
        } catch {
            # Fallback if gh repo view fails
        }
        
    } catch {
        Write-Host "❌ Failed to push test commit: $($_.Exception.Message)" -ForegroundColor Red
    }
}
...
if ($Interactive) {
    ...
    Write-Host ""
    $summaryColor = if ($successCount -eq $totalCount) { "Green" } else { "Yellow" }
    Write-Host "📊 Summary: $successCount/$totalCount secrets configured successfully" -ForegroundColor $summaryColor
    ...
    Write-Host "   .\scripts\setup-secrets-fixed.ps1 -Interactive" -ForegroundColor Green
    Write-Host ""
    ...
    Write-Host "   .\scripts\setup-secrets-fixed.ps1 -Test" -ForegroundColor Green
}
